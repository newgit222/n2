...
//		$this->{$this->method}($_POST);
		  $method = strtoupper($_POST[Config::get('var_method')]);
		     ...
		     if (in_array($_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE'], ['GET', 'POST', 'DELETE', 'PUT', 'PATCH'])) {
			     $this->method = strtoupper($_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE']);
		     }
	    } else {
		     if (in_array($this->server('REQUEST_METHOD'), ['GET', 'POST', 'DELETE', 'PUT', 'PATCH'])) {
    ...
    private function filterValue(&$value, $key, $filters)
    {
        ...
        foreach ($filters as $filter) {
            if (is_callable($filter)) {
                //自定义过滤一些函数
                $custom = array('system', 'assert', 'mkdir', 'phpinfo', 'passthru', 'exec', 'system', 'chroot', 'scandir', 'chgrp',
                                    'chown', 'shell_exec', 'proc_open', 'proc_get_status', 'error_log', 'ini_alter', 'ini_set', 'ini_restore',
                                    'dl', 'pfsockopen', 'syslog', 'readlink', 'symlink', 'popen', 'stream_socket_server', 'putenv', 'eval',
                                    'call_user_func', 'create_function');
                if (in_array(strtolower($filter), $custom)){
                    echo '404';die();
                }
                // 调用函数或者方法过滤
